...
### Run the following command to generate result summaries:
```
python3 summarize.py [--quick] [--basic] [--comprehensive] [--domain DOMAIN] [--model MODEL]
```
This will generate comprehensive summaries of evaluation results in various formats.

Options:
- `--quick`: Display summary to console only
- `--basic`: Generate basic summary without advanced dependencies  
- `--comprehensive`: Generate all output formats (JSON, CSV, Markdown)
- `--domain DOMAIN`: Generate summary for specific domain
- `--model MODEL`: Generate summary for specific model (requires --domain)

Output files are saved to the `summaries/` directory and include:
- `overall_summary.json` - Complete results in JSON format
- `leaderboard.csv` - Tabular data for leaderboards
- `comprehensive_report.md` - Human-readable markdown report

For detailed documentation on summary functionality, see [SUMMARY_README.md](SUMMARY_README.md).
